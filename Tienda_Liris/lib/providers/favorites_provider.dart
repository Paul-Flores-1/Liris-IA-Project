...
class FavoritesProvider extends ChangeNotifier {
  ...
  // Esta es la que llamaremos desde el catálogo
  Future<void> toggleFavorite(Perfume perfume) async {
    if (isFavorite(perfume.id)) {
      await removeFavorite(perfume.id);
    } else {
      await addFavorite(perfume);
    }
  }
  ...
  Future<void> addFavorite(Perfume perfume) async {
    ...
    try {
      await _userFavoritesCollection.doc(perfume.id).set({
        ...
        'sexo': perfume.sexo,
        'ml': perfume.ml, 
        'notas_olfativas': perfume.notasOlfativas,
