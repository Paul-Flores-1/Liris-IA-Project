        ...
        slivers: [
          ...
          StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              if (snapshot.hasError) return const SliverToBoxAdapter(child: Center(child: Text("Error")));
              if (snapshot.connectionState == ConnectionState.waiting) {
                return SliverToBoxAdapter(child: Center(child: CircularProgressIndicator(color: colorDorado)));
              }

              List<Perfume> perfumes = snapshot.data!.docs.map((doc) => Perfume.fromFirestore(doc)).toList();
              
              if (_selectedMarca != 'Todas') {
                perfumes = perfumes.where((p) => p.marca.toLowerCase().contains(_selectedMarca.toLowerCase())).toList();
              }
              if (_searchController.text.isNotEmpty) {
                final query = _searchController.text.toLowerCase();
                perfumes = perfumes.where((p) => p.nombre.toLowerCase().contains(query) || p.marca.toLowerCase().contains(query)).toList();
              }
                  ...
                  Container(
                    ...
                    child: Hero(
                      ...
                      child: perfume.imagenUrl.isNotEmpty
                          ? Image.network(perfume.imagenUrl, fit: BoxFit.contain)
                          : const Icon(Icons.image_not_supported, color: Colors.white24, size: 40),
                    ),
                  ),
                  // --- FIX: BOTÓN DE FAVORITOS CORREGIDO ---
                  Positioned(
                    top: 8,
                    right: 8,
                    child: Consumer<FavoritesProvider>(
                      builder: (context, provider, _) {
                        ...
                        return Material( // Necesario para que el splash del botón se vea bien
                          color: Colors.transparent,
                          child: InkWell(
                            onTap: () {
                              provider.toggleFavorite(perfume);
                            },
                ...
                child: Column(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(perfume.marca.toUpperCase(), style: TextStyle(color: Colors.white.withAlpha(150), fontSize: 10, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 4),
                        Text(perfume.nombre, style: const TextStyle(color: Colors.white, fontSize: 13, height: 1.2), maxLines: 2, overflow: TextOverflow.ellipsis),
                      ],
                    ),
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("\$${currencyFormat.format(perfume.precio)}", style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 4),
                        const Text("Envío gratis", style: TextStyle(color: Color(0xFF00A650), fontSize: 11, fontWeight: FontWeight.w700)),
                      ],
                    ),
                  ],
                ),
